...
describe 'seat_geek' do
  describe '.get_events' do
    before(:each) do
      options = {travel_dates: {arrive: '2016-08-01', depart: '2016-08-28'}, state: 'ny', attendee_count: '2', event_type: 'mlb' }
    ...
    it 'should return december as month' do
      expect(@result.first["datetime_utc"]).to match(/2016-08/)
    end

    it 'should return nba event' do
      expect(@result.first['type']).to match(/mlb/)
    end
  end
